...
class SpeedySpeechConfig(BaseTTSConfig):
    ...
    Example:
        ...
        >>> from TTS.TTS.tts.configs import SpeedySpeechConfig
    ...
    Args:
        ...
        encoder_type (str):
            Type of the encoder used by the model. Look at `TTS.TTS.tts.layers.feed_forward.encoder` for more details.
            Defaults to `residual_conv_bn`.
        encoder_params (dict):
            Parameters used to define the encoder network. Look at `TTS.TTS.tts.layers.feed_forward.encoder` for more details.
            Defaults to `{"kernel_size": 4, "dilations": [1, 2, 4, 1, 2, 4, 1, 2, 4, 1, 2, 4, 1], "num_conv_blocks": 2, "num_res_blocks": 13}`
        decoder_type (str):
            Type of the decoder used by the model. Look at `TTS.TTS.tts.layers.feed_forward.decoder` for more details.
            Defaults to `residual_conv_bn`.
        decoder_params (dict):
            Parameters used to define the decoder network. Look at `TTS.TTS.tts.layers.feed_forward.decoder` for more details.
