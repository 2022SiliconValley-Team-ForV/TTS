...
class GlowTTSConfig(BaseTTSConfig):
    """Defines parameters for GlowTTS model.
     ...
     Example:
        ...
        >>> from TTS.TTS.tts.configs import GlowTTSConfig
    ...
    Args:
        ...
        encoder_type (str):
            Type of the encoder used by the model. Look at `TTS.TTS.tts.layers.glow_tts.encoder` for more details.
            Defaults to `rel_pos_transformers`.
        encoder_params (dict):
            Parameters used to define the encoder network. Look at `TTS.TTS.tts.layers.glow_tts.encoder` for more details.
